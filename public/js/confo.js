...
EnxRtc.getDevices(function (arg) {
    ...
    let camlist = '';
    if (arg.result === 0) {
        document.getElementById("mic_cam_list").display = "block";
        arg.devices.mic.forEach(element => {
            ...
            if (element.deviceId === "default") {
                ...
                list += `<input type="radio" id="${element.deviceId}" name="fav_language" value="${element.label}" onclick="switchmicro(this)" checked> <label for="${element.deviceId}">${element.label}</label><br>`
            }
            else {
                list += `<input type="radio" id="${element.deviceId}" name="fav_language" value="${element.label}" onclick="switchmicro(this)"> <label for="${element.deviceId}">${element.label}</label><br>`
            }
            // list += `<li id="${element.deviceId}" style="display:flex;justify-content: space-between;align-items: center;">${element.label}</li>`;
        });
        document.querySelector('.miclist').innerHTML = list;
        arg.devices.cam.forEach(element => {
            var camId = element.deviceId.toString();
            console.log("====================================" + camId);
            console.log("=============================" + element.deviceId);

            camlist += `<input type="radio" id="${element.deviceId}" name="fav_languag" value="${element.label}" onclick="switchcam(this)"> <label for="${element.deviceId}">${element.label}</label><br>`

            // list += `<li id="${element.deviceId}" style="display:flex;justify-content: space-between;align-items: center;">${element.label}</li>`;
        });
        document.querySelector('.cameralist').innerHTML = camlist;
...
function miclist() {
    if (toggle === 0) {
        document.querySelector('.device_container').setAttribute("style", "display:flex;flex-direction:column;width: 450px;position: absolute;right: 0px;background-color: white; z-index: 1000000000000000;padding:5px;");
        toggle = 1;
    }
    else {
        document.querySelector('.device_container').setAttribute("style", "display: none ;width: 450px;position: absolute;right: 0px;background-color: white; z-index: 1000000000000000;");
        toggle = 0;
    }
}

function switchmicro(_this) {
    // console.log("outside this id is ====================>"+ _this.id);
    localStream.switchMicrophone(localStream, _this.id, function (Stream) {
        // console.log("inside this id is ==================>" + _this.id);
        if (Stream && Stream.getID) {
            // console.log("stream id =======================> " + Stream + JSON.stringify(Stream));
            // console.log("Inside switchmicro===================>"+localStream);
            localStream = Stream; // LocalStream updated   
        }
        else {
            console.log("it is failed=====================================>");
            // Failed to switch
            // switchmicro(localStream.config.audio);
        }
    });
}

function switchcam(_this) {
    console.log("outside switcam =======================");
    console.log("id is -=========================" + _this.id);
    localStream.switchCamera(localStream, _this.id, function (Stream) {
        console.log("dobara aaagaya======================");
        if (Stream && Stream.getID) {
            console.log("inside switchcam =======================");
            localStream = Stream; // LocalStream updated   
        }
        else {
            console.log("switchcam error======================" + JSON.stringify(Stream) + Stream);
            // Failed to switch
        }
    });
}
