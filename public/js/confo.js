...
let toggle_case = false;
let check_one_obj = 1;
let toggle_board = 0;
let toggle_share = false;
...
function ConnectCall(token) {
    ...
    localStream = EnxRtc.joinRoom(token, config, function (success, error) {
        ...
        if (success && success !== null) {
            ...
            //only moderator can see the whiteboard button.
            if (!isModerator) {
                document.querySelector('#white_board_btn').style.display = 'none';
            }
...
function whiteboard() {
    if (toggle_board === 0) {
        ...
        document.querySelector('#white_b').style.display = 'none';
        toggle_board = 0;
    }
}
